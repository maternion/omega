...
├── agent/                     # multi-turn loop (AgentLoop interface + DefaultAgentLoop), tool execution, compaction (config + token estimation), capability seams (Compactor, ToolProvider, AgentLoop, StoreProvider, SkillsProvider), extensions; events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, AssistantMessageEvent, ToolResultEvent, SessionEvent
...
| `agent/`          | Implemented | Multi-turn loop (`AgentLoop` interface + `DefaultAgentLoop`), tool execution (per-path file locks), compaction (config + token estimation, overflow auto-retry, friendly error when no compactor extension), capability seams (`Compactor`, `ToolProvider`, `AgentLoop`, `StoreProvider`, `SkillsProvider`), extensions (`StdioManager`, `ExtensionManager`, `ProxyStore`, `ProxySkills`, `ProxyCompactor`, `InjectedMessage`, `PendingDelegations`); events: AgentStart, TurnStart, TurnEnd, AgentEnd, StreamEvent, ToolResultEvent, SessionEvent; shared data types (Session, SessionNode, Insights, SearchResult) |
