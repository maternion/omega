...
| `extensions/` | Implemented | In-process extension packages: agent_loop (default conversation loop), provider (Ollama, OpenAI, Anthropic), store (SQLite + sessions.search), skills (skill loading + /skills command), compactor (context compaction), logging (file logger + LoggerProvider seam), memory (persistent memory + memory tool), prompt (system prompt builder), tools (shell, files), mcp (MCP bridge), delegate (subagent delegation), web (web search/fetch). Each implements the `agent.Plugin` interface.                                                                                                                                                                                               |
